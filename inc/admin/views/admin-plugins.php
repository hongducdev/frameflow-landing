...
$status_ins = false;
$status_act = false;
$btn_text = esc_html__('Install All', 'frameflow');
...
$merlin_setup = get_option('merlin_' . frameflow()->get_slug() . '_completed');

if ($status_ins && $status_act)
	$btn_text = esc_html__('Install & Active All', 'frameflow');
else if ($status_ins && !$status_act)
	$btn_text = esc_html__('Install All', 'frameflow');
else if (!$status_ins && $status_act)
	$btn_text = esc_html__('Active All', 'frameflow');
?>
<main>
		...
		if ('valid' != get_option(frameflow()->get_slug() . '_purchase_code_status', false) && !$dev_mode) :
			...
			<header class="pxl-dsb-header admin-plugin">
				<div class="pxl-dsb-header-inner">
					<h4><?php esc_html_e('Install Plugins', 'frameflow'); ?></h4>
				...
				<p><?php esc_html_e('Make sure to activate required plugins prior to import a demo.', 'frameflow'); ?></p>
			</header>
				...
				<div class="pxl-row">
					...
					foreach ($plugins as $plugin) :
						...
						elseif (!is_plugin_inactive($file_path)) {
							...
							$display_status = esc_html__('Active:', 'frameflow');
						}
					?>
						<div class="pxl-col pxl-col-3">
							<div class="pxl-dsb-plugin<?php echo esc_attr($class); ?>" data-slug="<?php echo esc_attr($plugin['slug']) ?>">
								...
								<?php
								$barplugin = new Frameflow_Admin_Plugins;
