...
function frameflow_html($html)
{
    return $html;
}

/**
 * Font family string from a Redux typography option array.
 *
 * @param mixed  $opt     Theme option value.
 * @param string $fallback Fallback family.
 * @return string
 */
function frameflow_typography_family($opt, $fallback = '')
{
    if (is_array($opt) && !empty($opt['font-family'])) {
        return (string) $opt['font-family'];
    }
    return $fallback;
}

/**
 * Build a CSS font-family stack from a family name.
 *
 * @param string $family  Font family name.
 * @param string $generic Generic fallback (serif|sans-serif).
 * @return string
 */
function frameflow_font_stack($family, $generic = 'sans-serif')
{
    $family = trim((string) $family);
    if ($family === '') {
        return $generic;
    }
    if (strpos($family, ' ') !== false && $family[0] !== '"') {
        $family = '"' . $family . '"';
    }
...
/**
 * Resolve active body / heading / theme-default font stacks from Theme Options.
 *
 * @return array{body: string, heading: string, theme_default: string, families: string[]}
 */
function frameflow_resolved_fonts()
{
    $body_type = frameflow()->get_theme_opt('pxl_body_typography', 'default-font');
    $heading_type = frameflow()->get_theme_opt('pxl_heading_typography', 'default-font');

    $body_family = 'Inter';
    $heading_family = 'Instrument Serif';
    $theme_default_family = 'Schibsted Grotesk';
    $extra = [];

    if ($body_type === 'google-font') {
        $picked = frameflow_typography_family(frameflow()->get_theme_opt('font_body', []));
        if ($picked !== '') {
            $body_family = $picked;
            $extra[] = $picked;
        }
    ...
    if ($heading_type === 'google-font') {
        $picked = frameflow_typography_family(frameflow()->get_theme_opt('font_heading', []));
        if ($picked !== '') {
            $heading_family = $picked;
            $extra[] = $picked;
        }
    }

    $theme_default = frameflow()->get_theme_opt('theme_default', []);
    $picked_default = frameflow_typography_family($theme_default);
    if ($picked_default !== '') {
        $theme_default_family = $picked_default;
        $extra[] = $picked_default;
    }

    return [
        'body' => frameflow_font_stack($body_family, 'sans-serif'),
        'heading' => frameflow_font_stack($heading_family, 'serif'),
        'theme_default' => frameflow_font_stack($theme_default_family, 'sans-serif'),
        'families' => array_values(array_unique($extra)),
    ];
}

/**
 * Google Fonts URL — base theme fonts + any Theme Options Google families.
 */
function frameflow_fonts_url()
{
    $fonts = [];

    // Always load design-system defaults (fallbacks when options = Default Font).
    $fonts[] = 'Schibsted+Grotesk:ital,wght@0,400..900;1,400..900';
    $fonts[] = 'Inter:ital,opsz,wght@0,14..32,100..900;1,14..32,100..900';
    $fonts[] = 'Instrument+Serif:ital@0;1';

    $resolved = frameflow_resolved_fonts();
    foreach ($resolved['families'] as $family) {
        $slug = str_replace(' ', '+', $family);
        // Skip families already in the base list.
        if (stripos($slug, 'Schibsted') === 0 || stripos($slug, 'Inter') === 0 || stripos($slug, 'Instrument') === 0) {
            continue;
        }
        $fonts[] = $slug . ':ital,wght@0,300;0,400;0,500;0,600;0,700;1,400';
    }
 ...
 * Get page ID by Slug
 */
function frameflow_get_id_by_slug($slug, $post_type)
{
 ...
 **/
function frameflow_content_by_slug($slug, $post_type)
{
    $content = frameflow_get_content_by_slug($slug, $post_type);

    $id = frameflow_get_id_by_slug($slug, $post_type);
 ...
 * Get content by slug
 **/
function frameflow_get_content_by_slug($slug, $post_type)
{
    $content = get_posts(
        array(
            'name'      => $slug,
 ...
 * Custom Comment List
 */
function frameflow_comment_list($comment, $args, $depth)
{
    if ('div' === $args['style']) {
        $tag       = 'div';
        $add_below = 'comment';
...
?>
    <<?php echo '' . $tag ?> <?php comment_class(empty($args['has_children']) ? '' : 'parent') ?> id="comment-<?php comment_ID() ?>">
                    ...
                    <div class="comment--meta">
                        <div class="comment--info">
                            ...
                            <div class="comment--date">
                                <span class="comment--date-inner">
                                    <?php echo esc_html('Commented at', 'frameflow'); ?>
                                    <?php echo get_comment_time() . ' ' . get_comment_date('F j, Y'); ?>
                                </span>
                            </div>
                        </div>
         ...
         * Paginate Links
         */
        function frameflow_ajax_paginate_links($link)
        {
         ...
         * RGB Color
         */
        function frameflow_hex_rgb($color)
        {
         ...
         * Image Size Crop
         */
        if (!function_exists('frameflow_get_image_by_size')) {
            function frameflow_get_image_by_size($params = array())
            {
        ...
        {
            $search_mobile = frameflow()->get_theme_opt('search_mobile', false);
            $search_placeholder_mobile = frameflow()->get_theme_opt('search_placeholder_mobile');
            if ($search_mobile) : ?>
        ...
        if (function_exists('pxl_register_shortcode')) {
            function frameflow_year_shortcode()
            {
                ob_start(); ?>
            ...
            pxl_register_shortcode('pxl_year', 'frameflow_year_shortcode');
        }

        /* Highlight Shortcode  */
        if (function_exists('pxl_register_shortcode')) {
            function frameflow_text_highlight_shortcode($atts = array())
            {
            ...
            pxl_register_shortcode('highlight', 'frameflow_text_highlight_shortcode');
        }

        if (function_exists('pxl_register_shortcode')) {
            function frameflow_text_highlight_box_shortcode($atts = array())
            {
                extract(shortcode_atts(array(
                    'text' => '',
                    'style' => '',
                ...
                ob_start();
                if (!empty($text)) : ?>
                <span class="pxl-title--highlight-box <?php echo esc_attr($style); ?>">
                    <span class="pxl-title--highlight-box-text">
                        <?php echo wp_kses_post($text); ?>
                    </span>
                    <span class="pxl-title--highlight-box-left">
                    </span>
                    <span class="pxl-title--highlight-box-right">
                    </span>
                </span>
            <?php endif;
                $output = ob_get_clean();
            ...
            pxl_register_shortcode('highlight_box', 'frameflow_text_highlight_box_shortcode');
        }

        /* Image Shortcode */
        if (function_exists('pxl_register_shortcode')) {
            function frameflow_image_shortcode($atts = array())
            {
                ...
                if (!empty($atts['id'])) {
                    $img_html = wp_get_attachment_image(
                        ...
                        array(
                            ...
                            'class' => trim('pxl-shortcode-image ' . $atts['class']),
                            'loading' => 'lazy',
                        )
                    );
                } elseif (!empty($atts['src'])) {
                    $img_html = sprintf(
                        '<img src="%1$s" alt="%2$s" class="pxl-shortcode-image %3$s" loading="lazy" />',
                        esc_url($atts['src']),
                        esc_attr($atts['alt']),
                        esc_attr($atts['class'])
                    );
                ...
                if (!empty($atts['link'])) {
                    ...
                    $img_html = sprintf(
                        '<a href="%1$s" target="%2$s"%4$s>%3$s</a>',
                        esc_url($atts['link']),
            ...
            pxl_register_shortcode('pxl_image', 'frameflow_image_shortcode');
        }

        if (function_exists('pxl_register_shortcode')) {
            function frameflow_text_highlight_shortcode_editor($atts = array())
            {
                extract(shortcode_atts(array(
                    'text' => '',
                ), $atts));
            ...
            pxl_register_shortcode('pxl_highlight', 'frameflow_text_highlight_shortcode_editor');
        }

        /* Typewriter Shortcode  */
        if (function_exists('pxl_register_shortcode')) {
            function frameflow_text_typewriter_shortcode($atts = array())
            {
                extract(shortcode_atts(array(
                    'text' => '',
                ), $atts));
            ...
            pxl_register_shortcode('typewriter', 'frameflow_text_typewriter_shortcode');
        }

        /* Button Shortcode  */
        if (function_exists('pxl_register_shortcode')) {
            function frameflow_btn_shortcode($atts = array())
            {
                extract(shortcode_atts(array(
                    'text' => '',
                    'style' => '',
            ...
            pxl_register_shortcode('pxl_button', 'frameflow_btn_shortcode');
        }

        if (function_exists('pxl_register_shortcode')) {
            function frameflow_btn_video_shortcode($atts = array())
            {
                extract(shortcode_atts(array(
                    'text' => '',
                    'url' => '',
            ...
            pxl_register_shortcode('pxl_btn_video', 'frameflow_btn_video_shortcode');
        }


        if (function_exists('pxl_register_shortcode')) {
            function frameflow_btn_submit_shortcode($atts = array())
            {
                extract(shortcode_atts(array(
                    'text' => '',
            ...
            pxl_register_shortcode('pxl_button_submit', 'frameflow_btn_submit_shortcode');
        }

        if (function_exists('pxl_register_shortcode')) {
            function frameflow_slider_arrow($atts = array())
            {
                extract(shortcode_atts(array(
                    'type' => 'next',
                    'style' => 'style-1',
        ...
        <?php $output = ob_get_clean();
            ...
            pxl_register_shortcode('slider_arrow', 'frameflow_slider_arrow');
        }

        // Shortcode Row/Column Grid
        if (function_exists('pxl_register_shortcode')) {
            function frameflow_start_row_shortcode($atts = array())
            {
                ob_start(); ?>
            ...
            pxl_register_shortcode('pxl_start_row', 'frameflow_start_row_shortcode');
        }

        if (function_exists('pxl_register_shortcode')) {
            function frameflow_end_row_shortcode($atts = array())
            {
                ob_start(); ?>
                </div>
            </div>
        <?php $output = ob_get_clean();
            ...
            pxl_register_shortcode('pxl_end_row', 'frameflow_end_row_shortcode');
        }

        if (function_exists('pxl_register_shortcode')) {
            function frameflow_start_col_shortcode($atts = array())
            {
                extract(shortcode_atts(array(
                    'class' => 'col-12',
                ), $atts));
            ...
            pxl_register_shortcode('pxl_start_column', 'frameflow_start_col_shortcode');
        }

        if (function_exists('pxl_register_shortcode')) {
            function frameflow_end_col_shortcode($atts = array())
            {
                ob_start(); ?>
            </div>
        <?php $output = ob_get_clean();
            ...
            pxl_register_shortcode('pxl_end_column', 'frameflow_end_col_shortcode');
        }
        ...
        /* Gallery Shortcode  */
        if (function_exists('pxl_register_shortcode')) {
            function frameflow_gallery_shortcode($atts = array())
            {
                extract(shortcode_atts(array(
                    'link_video' => '',
                    'images_id' => '',
                ...
                ?>
                    <div class="pxl--item">
                        <div class="pxl--item-inner">
                            <a href="<?php echo esc_url($thumbnail_url); ?>" data-elementor-lightbox-slideshow="pxl-gallery-<?php echo esc_attr($pxl_g_id); ?>"><?php echo frameflow_html($thumbnail); ?></a>
        ...
        <?php
            ...
            pxl_register_shortcode('pxl_gallery', 'frameflow_gallery_shortcode');
        }

        /* Addd shortcode Video button */
        if (function_exists('pxl_register_shortcode')) {
            function frameflow_video_button_shortcode($atts = array())
            {
                extract(shortcode_atts(array(
                    'link' => '',
                    'text' => '',
        ...
        <?php
            ...
            pxl_register_shortcode('pxl_video_button', 'frameflow_video_button_shortcode');
        }

        /* Get Category Shortcode  */
        if (function_exists('pxl_register_shortcode')) {
            function frameflow_post_category_shortcode($atts = array())
            {
                extract(shortcode_atts(array(
                    'items' => '6',
                    'columns' => '2',
            ...
            pxl_register_shortcode('pxl_post_category', 'frameflow_post_category_shortcode');
        }

        /* Slider 1  */
        if (function_exists('pxl_register_shortcode')) {
            function frameflow_slider_price_shortcode($atts = array())
            {
                extract(shortcode_atts(array(
                    'price' => '',
                    'desc' => '',
            ...
            pxl_register_shortcode('pxl_slider_price', 'frameflow_slider_price_shortcode');
        }

        /**
         * Custom Widget Categories - Count
         */
        add_filter('wp_list_categories', 'frameflow_wg_category_count');
        function frameflow_wg_category_count($output)
        {
         ...
         */
        add_filter('get_archives_link', 'frameflow_wg_archive_count');
        function frameflow_wg_archive_count($links)
        {
         ...
         */
        add_filter('wp_list_categories', 'frameflow_wc_cat_count_span');
        function frameflow_wc_cat_count_span($links)
        {
            $dir = is_rtl() ? 'pxl-left' : 'pxl-right';
         ...
         */
        function frameflow_get_mega_menu_builder_id()
        {
            $mn_id = [];
         ...
         */
        function frameflow_get_page_popup_builder_id()
        {
            $pp_id = [];
        ...
        function frameflow_mouse_move_animation()
        {
            $mouse_move_animation = frameflow()->get_theme_opt('mouse_move_animation', false);
            if ($mouse_move_animation) {
                wp_enqueue_script('frameflow-cursor', get_template_directory_uri() . '/assets/js/libs/cursor.min.js', array('jquery'), '1.0.0', true); ?>
         ...
         * Start - Cookie Policy
         */
        function frameflow_cookie_policy()
        {
            $cookie_policy = frameflow()->get_theme_opt('cookie_policy', 'hide');
            $cookie_policy_description = frameflow()->get_theme_opt('cookie_policy_description');
            $cookie_policy_btntext = frameflow()->get_theme_opt('cookie_policy_btntext');
            $cookie_policy_link = get_permalink(frameflow()->get_theme_opt('cookie_policy_link'));
    ...
    <?php }
         ...
         */
        function frameflow_subscribe_popup()
        {
            $subscribe = frameflow()->get_theme_opt('subscribe', 'hide');
            $subscribe_layout = frameflow()->get_theme_opt('subscribe_layout');
            $popup_effect = frameflow()->get_theme_opt('popup_effect', 'fade');
            $args = [
    ...
    <?php }
         ...
         */
        add_action('show_user_profile', 'frameflow_user_fields');
        add_action('edit_user_profile', 'frameflow_user_fields');
        function frameflow_user_fields($user)
        {
    ...
    ?>
        <h3><?php esc_html_e('Theme Custom', 'frameflow'); ?></h3>
        <table class="form-table">
            <tr>
                <th><label for="user_name"><?php esc_html_e('Author Name', 'frameflow'); ?></label></th>
                <td>
            ...
            <tr>
                <th><label for="user_position"><?php esc_html_e('Author Position', 'frameflow'); ?></label></th>
                <td>
        ...
        function frameflow_get_user_name()
        {
        ...
        function frameflow_get_user_position()
        {
        ...
        function frameflow_attachment_field_credit($form_fields, $post)
        {
        ...
        add_filter('attachment_fields_to_edit', 'frameflow_attachment_field_credit', 1, 2);


        function frameflow_attachment_field_credit_save($post, $attachment)
        {
        ...
        add_filter('attachment_fields_to_save', 'frameflow_attachment_field_credit_save', 1, 2);
        /* End Custom Field Media */

        /* Author Social */
        function frameflow_get_user_social()
        {
    ...
    <?php }
        ...
        /**
         * Fix for "Attempt to read property on null" warnings in WordPress Admin / Cron
         * occurring when WC_Admin_List_Table_Products::render_name_column() is called
         * without global $post being set up correctly by the caller.
         */
        add_action('manage_product_posts_custom_column', function ($column, $post_id) {
            global $post;
            if ((empty($post) || ! ($post instanceof WP_Post)) && ! empty($post_id)) {
