...
if (!class_exists('Frameflow_Main')) {
    class Frameflow_Main extends Frameflow_Base
    {
        ...
        public static function getInstance()
        {
            ...
            if (null === self::$instance) {
                self::$instance = new Frameflow_Main();
            }
        ...
        public function require_folder($foldername, $path = '')
        {
            ...
            foreach (glob($dir . DIRECTORY_SEPARATOR . '*.php') ?: [] as $patch) {
                require_once $patch;
            }
        }
            ...
            if ($page_opt !== NULL && $page_opt !== '' && $page_opt !== '-1') {
                if (is_array($page_opt) && is_array($theme_opt)) {
                    foreach ($page_opt as $key => $value) {
                        if ((empty($page_opt[$key]) || $page_opt[$key] === 'px') && isset($theme_opt[$key])) {
        ...
        public function get_sidebar_args($args = [])
        {
            ...
            if (!empty($args['force_sidebar'])) {
                $sidebar_active = true;
            }

            $default_pos = $args['type'] == 'page' ? '0' : 'right';
            $sidebar_pos = in_array($args['type'], ['blog', 'post', 'shop'], true)
                ? $default_pos
                : $this->get_opt($args['type'] . '_sidebar_pos', $default_pos);
...
function frameflow()
{
    return Frameflow_Main::getInstance();
}
// Install
frameflow();
