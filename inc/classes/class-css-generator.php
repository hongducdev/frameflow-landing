...
class Frameflow_CSS_Generator
{
    ...
    {
        $this->opt_name = frameflow()->get_option_name();
        ...
        add_action('wp_enqueue_scripts', array($this, 'frameflow_enqueue'), 20);
    }
    ...
    function init()
    {
        ...
        add_action('wp', array($this, 'frameflow_generate_with_dev_mode'));
        add_action("redux/options/{$this->opt_name}/saved", function () {
            $this->frameflow_generate_file_options();
        });
    }

    function frameflow_generate_with_dev_mode()
    {
        if ($this->dev_mode === true) {
            $this->frameflow_generate_file_options();
            $this->frameflow_generate_file();
        }
    }

    function frameflow_generate_file_options()
    {
        ...
        $this->redux->filesystem->execute('put_contents', $_options, array(
            'content' => preg_replace("/(?<=[^\r]|^)\n/", "\r\n", $this->frameflow_options_output())
        ));
    }

    function frameflow_generate_file()
    {
        $scss_dir = get_template_directory() . '/assets/scss/';
    ...
    protected function frameflow_options_output()
    {
        $theme_colors                    = frameflow_configs('theme_colors');
        $links                           = frameflow_configs('link');
        $gradients                       = frameflow_configs('gradient');
        $gradient_two                    = frameflow_configs('gradient_two');
        ob_start();

        printf('$frameflow_theme_colors_key:(%s);', $this->print_scss_opt_colors($theme_colors, 'key'));
        printf('$frameflow_theme_colors_val:(%s);', $this->print_scss_opt_colors($theme_colors, 'val'));
        // color rgb only
        ...
        // gradient two color
        foreach ($gradient_two as $key => $value) {
            printf('$gradient_two_%1$s: %2$s;', str_replace('-', '_', $key), 'var(--gradient-two-' . $key . ')');
        }

        /* Font — runtime CSS vars win; SCSS mixin reads --theme-default-font */
        $theme_default = frameflow()->get_theme_opt('theme_default');
        $family = is_array($theme_default) && !empty($theme_default['font-family'])
            ? $theme_default['font-family']
            : '';
        if ($family !== '') {
            echo '$ft_theme_default: "google";';
            echo '$ft_theme_google: "' . esc_attr($family) . '";';
        } else {
            echo '$ft_theme_default: "Schibsted Grotesk";';
            echo '$ft_theme_google: "Inter";';
        }
    ...
    /* Inline CSS */
    function frameflow_enqueue()
    {
        $css = $this->frameflow_inline_css();
        if (!empty($css)) {
            wp_add_inline_style('pxl-style', $css);
        }
    }
    protected function frameflow_inline_css()
    {
        ob_start();
...
<?php
        $mobile_bg_light = frameflow()->get_opt('mobile_header_bg_light', '');
        $mobile_bg_dark  = frameflow()->get_opt('mobile_header_bg_dark', '');
        if (!empty($mobile_bg_light) || !empty($mobile_bg_dark)) :
            ?>
        @media screen and (max-width: 1200px) {
        <?php
            if (!empty($mobile_bg_light)) {
                printf('#pxl-header-elementor #pxl-header-mobile.style-light { background-color: %s; }', esc_attr($mobile_bg_light));
                printf('#pxl-header-elementor #pxl-header-mobile.style-light.pxl-header-mobile-fixed { background-color: %s; }', esc_attr($mobile_bg_light));
            }
            if (!empty($mobile_bg_dark)) {
                printf('#pxl-header-elementor #pxl-header-mobile.style-dark { background-color: %s; }', esc_attr($mobile_bg_dark));
                printf('#pxl-header-elementor #pxl-header-mobile.style-dark.pxl-header-mobile-fixed { background-color: %s; }', esc_attr($mobile_bg_dark));
            }
            ?>
        }
            <?php
        ...
        $logo_height_css = '';
        if (frameflow()->get_opt('mobile_display', 'show') === 'show') {
            $logo_height = frameflow()->get_opt('logo_height', []);
            if (is_array($logo_height) && !empty($logo_height['height'])) {
                $height_value = trim((string) $logo_height['height']);
                $height_unit = !empty($logo_height['units']) ? trim((string) $logo_height['units']) : 'px';

                if ($height_value !== '') {
                    if (preg_match('/[a-z%]+$/i', $height_value)) {
                        $logo_height_css = $height_value;
                    } else {
                        $logo_height_css = $height_value . $height_unit;
                    }
                }
            }
        }
        ...
        <?php
            printf(
                '#pxl-header-default .pxl-header-branding img,#pxl-header-elementor #pxl-header-mobile .pxl-header-branding img,.pxl-logo-mobile img,#pxl-header-elementor .pxl-header-mobile-elementor .pxl-logo img{height:%1$s !important;width:auto;}',
                esc_attr($logo_height_css)
            );
            ?>
        }
            <?php
