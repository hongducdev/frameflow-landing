;(function ($) {
    ...
    function makeImgNode(url) {
        ...
        container.style.height = ICON_SIZE
    ...
    function applyItemBackground(el, bg) {
        if (bg && typeof bg === "object" && bg.type === "gradient") {
            var from = typeof bg.from === "string" ? bg.from.trim() : ""
            var to = typeof bg.to === "string" ? bg.to.trim() : ""
            if (!to) {
                to = from
            }
            if (from) {
        ...
        var slug = bg && typeof bg === "object" && bg.slug != null ? String(bg.slug).trim() : ""
        if (!slug && typeof bg === "string") {
            slug = bg.trim()
        }
        if (!ALLOWED_BG[slug]) {
            slug = "primary"
        }
        el.className = el.className + " " + BG_PREFIX + slug
    }
    ...
    function initPhysicsArea(logoArea) {
        if (typeof Matter === "undefined" || !logoArea || logoArea.nodeType !== 1) {
            return
        }
        ...
        try {
            ...
            var labels = parseDataJsonAttr(logoArea, "data-labels")
            var bgs = parseDataJsonAttr(logoArea, "data-bgs")
            if (!Array.isArray(bgs) || bgs.length < 1) {
                bgs = parseDataJsonAttr(logoArea, "data-bg-classes")
            }

            var itemCount = icons.length
            if (labels.length > itemCount) {
                itemCount = labels.length
            }
            if (bgs.length > itemCount) {
                ...
                window.requestAnimationFrame(function () {
                    window.requestAnimationFrame(function () {
                        ...
                        shapes.forEach(function (pair) {
                            ...
                            var bw = pair._hitW
                            var bh = pair._hitH
                            if (!bw || !bh || (Math.abs(mw - bw) < 1 && Math.abs(mh - bh) < 1)) {
                                return
                            }
                            var sx = mw / bw
                            var sy = mh / bh
            ...
            for (index = 0; index < itemCount; index++) {
                ...
                applyItemBackground(textElement, Array.isArray(bgs) ? bgs[index] : null)
                ...
                var label =
                    Array.isArray(labels) && typeof labels[index] === "string" ? labels[index] : ""
                if (isString(label)) {
                    var labelNode = document.createElement("span")
                    labelNode.className = "pxl-throwable-label"
                    labelNode.textContent = label
                    textElement.appendChild(labelNode)
                }
                ...
                var measuredWidth = Math.max(40, Math.ceil(textElement.offsetWidth))
                var measuredHeight = Math.max(24, Math.ceil(textElement.offsetHeight))
                ...
                var spawnY = minY + Math.random() * Math.max(24, maxY - minY)
                ...
                while (tries-- > 0) {
                    ...
                    for (pIndex = 0; pIndex < placedBodies.length; pIndex++) {
                        ...
                        var collideX = Math.abs(px - placed.x) < halfW + placed.halfW + spawnGap
                        var collideY = Math.abs(py - placed.y) < halfH + placed.halfH + spawnGap
