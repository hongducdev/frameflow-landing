    ...
    $(window).on("elementor/frontend/init", function () {
        setTimeout(function () {
            $(".pxl-grid").each(function (index, element) {
                ...
                $grid_scope.on(
                    "click",
                    ".pxl-project-filter--item .pxl-filter-placeholder",
                    function (e) {
                        e.preventDefault()
                        e.stopPropagation()
                        var $item = $(this).closest(".pxl-project-filter--item")
                        $grid_scope
                            .find(".pxl-project-filter--item")
                            .not($item)
                            .removeClass("is-open")
                ...
                $grid_scope.on(
                    "click",
                    ".pxl-project-filter--item .pxl-project-filter-menu a",
                    function (e) {
                        e.preventDefault()
                        e.stopPropagation()
                        var $link = $(this)
                        var $item = $link.closest(".pxl-project-filter--item")
                        var $placeholder = $item.find(".pxl-filter-placeholder")
                        var filterVal = $link.attr("data-filter") || ""
                        var label = $.trim($link.text())

                        $item
                            .find(".pxl-project-filter-menu a")
                            .removeClass("is-selected")
                        $link.addClass("is-selected")
                        $placeholder
                            .text(label)
                            .attr("data-filter", filterVal)
                        $item
                            .addClass("is-active")
                            .removeClass("is-open")
                    },
                ...
                $grid_scope.on(
                    ...
                    function (e) {
                        ...
                        $grid_isotope.isotope({
                            filter: parts.length ? parts.join("") : "*",
                        })
                        $grid_scope
                            .find(".pxl-project-filter--item")
                            .removeClass("is-open")
                ...
                $grid_scope.on("click", ".pxl-project-filter", function (e) {
                    e.stopPropagation()
                })
                ...
                $grid_scope.on(
                    ...
                    function (e) {
                        ...
                        if (
                            ...
                            frameflow_grid_ajax_handler(
                                $this,
                                $grid_scope,
                                $grid_isotope,
                                {
                                    action: "frameflow_load_more_post_grid",
                                    loadmore: loadmore,
            ...
            $(document)
                .off("click.pxlProjectFilter")
                .on("click.pxlProjectFilter", function () {
                    $(".pxl-project-filter--item").removeClass("is-open")
                })
        }, 300);

        function frameflow_grid_ajax_handler(
            $this,
            $grid_scope,
            $grid_isotope,
            args = {},
