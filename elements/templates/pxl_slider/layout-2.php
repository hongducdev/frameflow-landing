...
$slides =
    isset($settings['slides_2']) && is_array($settings['slides_2']) ? $settings['slides_2'] : [];

$has_slide = false;
foreach ($slides as $row) {
    if (
        (!empty($row['slide_template']) && absint($row['slide_template']) > 0) ||
        (!empty($row['slide_image']['id']) || !empty($row['slide_image']['url']))
    ) {
        $has_slide = true;
        break;
    }
...
?>
<div class="pxl-swiper-slider pxl-slider pxl-slider2"<?php if (
    $drap === 'yes'
): ?> data-cursor-drap="<?php echo esc_attr__('DRAG', 'frameflow'); ?>"<?php endif; ?>>
    <div class="pxl-carousel-inner">
        <div <?php pxl_print_html($widget->get_render_attribute_string('carousel')); ?>>
            <div class="pxl-swiper-wrapper">
                <?php foreach ($slides as $slide):

                    $template_id = isset($slide['slide_template'])
                        ? absint($slide['slide_template'])
                        : 0;
                    $image = isset($slide['slide_image']) ? $slide['slide_image'] : [];

                    $has_image = !empty($image['id']) || !empty($image['url']);
                    $slide_html = '';
                    if ($template_id > 0) {
                        $slide_html = Elementor\Plugin::$instance->frontend->get_builder_content_for_display(
                            $template_id,
                            true,
                        );
                    }

                    if (!$has_image && ($slide_html === '' || $slide_html === false)) {
                        continue;
                    }
                    ?>
                            ...
                            <div class="pxl-item--inner <?php echo esc_attr(
                                ...
                                <?php if ($has_image): ?>
                                    <div class="pxl-item--image">
                                        <?php echo pxl_print_html(
                                            wp_get_attachment_image($image['id'], 'full'),
                                        ); ?>
                                    </div>
                                <?php endif; ?>
                                <?php if ($slide_html !== '' && $slide_html !== false): ?>
