<?php if ($settings['search_type'] == 'popup') : ?>
	<div class="pxl-search-popup-button pxl-cursor--cta <?php echo esc_attr($settings['style']); ?>">
		<?php if (($settings['icon_image_type'] ?? 'img') === 'ic' && !empty($settings['pxl_icon']['value'])) {
			\Elementor\Icons_Manager::render_icon($settings['pxl_icon'], ['aria-hidden' => 'true', 'class' => ''], 'i');
		} else if (($settings['icon_image_type'] ?? 'img') === 'img' && !empty($settings['image']['id'])) {
	...
	<?php if (!has_action('pxl_anchor_target', 'frameflow_hook_anchor_search')) {
		add_action('pxl_anchor_target', 'frameflow_hook_anchor_search');
	} ?>
<?php endif; ?>
...
<?php if ($settings['search_type'] == 'form') : ?>
	<form role="search" method="get" class="pxl-widget-searchform" action="<?php echo esc_url(home_url('/')); ?>">
		<div class="searchform-wrap">
			<input type="text" placeholder="<?php if (!empty($settings['email_placefolder'])) {
				echo esc_attr($settings['email_placefolder']);
			} else {
