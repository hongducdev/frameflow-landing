;(function ($) {
    ...
    function frameflowExtractCssUrl(value) {
        if (!value || value === "none") return null
        var match = /url\(\s*['"]?([^"')]+)['"]?\s*\)/i.exec(value)
        return match ? match[1] : null
    }

    function frameflowWaterEffectImageUrl($el) {
        var el = $el[0]
        var nodes = [el]
        var overlay = el.querySelector(":scope > .elementor-background-overlay")
        var motion = el.querySelector(
            ":scope > .elementor-motion-effects-container .elementor-motion-effects-layer"
        )
        if (overlay) nodes.push(overlay)
        if (motion) nodes.push(motion)

        var vars = ["--background-overlay", "--background-image", "--e-con-background-image"]
        var i, n, cs, url, v

        for (i = 0; i < nodes.length; i++) {
            n = nodes[i]
            cs = window.getComputedStyle(n)
            url = frameflowExtractCssUrl(cs.backgroundImage)
            if (url) return url
            for (v = 0; v < vars.length; v++) {
                url = frameflowExtractCssUrl(cs.getPropertyValue(vars[v]))
                if (url) return url
            }
        }
    ...
    function frameflowCleanupRipples($el) {
        var instance = $el.data("ripples")
        if (instance && typeof instance.destroy === "function") {
            instance.destroy()
            return
        }
        $el.children("canvas").remove()
        $el.removeClass("jquery-ripples").removeData("ripples")
    }

    function frameflowInitWaterEffect() {
        if (typeof $.fn.ripples !== "function") return

        $(".water-effect, .pxl-bg-water-effect").each(function () {
            var $el = $(this)
            if ($el.data("ripples")) return
            if ($el.innerWidth() < 2 || $el.innerHeight() < 2) return

            var imageUrl = frameflowWaterEffectImageUrl($el)
            if (!imageUrl) return

            if (!frameflowExtractCssUrl($el.css("backgroundImage"))) {
                $el.css({
            ...
            var interactive = $el.attr("data-interactive") !== "false"
            var dropRadius = parseFloat($el.attr("data-drop-radius")) || 20
            var perturbance = parseFloat($el.attr("data-perturbance"))
            if (isNaN(perturbance) || perturbance < 0) perturbance = 0.04
            var resolution = parseInt($el.attr("data-resolution"), 10) || 256
            var isFirefox = /Firefox\//i.test(navigator.userAgent)
            if (isFirefox) {
                resolution = Math.min(resolution, 256)
                perturbance = Math.max(perturbance, 0.06)
            }
            ...
            try {
                ...
                $el.children("canvas").css({
                    zIndex: 0,
                    background: "transparent",
                })
            } catch (err) {
                frameflowCleanupRipples($el)
            }
        })
    }

    window.frameflowInitWaterEffect = frameflowInitWaterEffect
    ...
    $(document).ready(function () {
        ...
        $(".pxl-check-scroll .pxl-swiper-slide .filter-item").on("mousedown", function () {
            $(this).closest(".pxl-swiper-slide").removeClass("visible").addClass("visible")
        })

        setTimeout(function () {
            var $rowsParticles = $(".pxl-row-particles")
            if (typeof particlesJS !== "function" || !$rowsParticles.length) return
        ...
        $(document).on("click.pxl_menu", ".pxl-menu-toggle", function (e) {
            ...
            if ($toggle.hasClass("active")) {
                $parentUl.find(".pxl-menu-toggle.active").not($toggle).toggleClass("active")
                $parentUl.find(".sub-menu.active").toggleClass("active").slideToggle()
            } else {
        ...
        const setMobileMenuState = function (isOpen) {
            ...
            if (isOpen) {
                ...
                const savedScrollTop = parseInt($body.data("pxl-mobile-scroll-top"), 10) || 0
                $body.css({
                    position: "",
                    top: "",
                    left: "",
        ...
        var $megaEl = $("li.pxl-megamenu > .sub-menu > .pxl-mega-menu-elementor")
        ...
        $(".pxl-subscribe-popup .pxl-item--overlay, .pxl-subscribe-popup .pxl-item--close").on(
            "click",
            function (e) {
        ...
        $("#pxl-search-popup .pxl-item--overlay, #pxl-search-popup .pxl-item--close").on(
            "click",
            function (e) {
        ...
        $(".btn-text-reverse .pxl-text--front, .btn-text-reverse .pxl-text--back").each(
            function () {
                var eltime = 0.05
                $(this)
        ...
        $("#pxl-cart-sidebar .pxl-popup--overlay, #pxl-cart-sidebar .pxl-item--close").on(
            "click",
            function () {
        ...
        wobbleElements.forEach(function (el) {
            el.addEventListener("mouseover", function () {
                if (!el.classList.contains("animating") && !el.classList.contains("mouseover")) {
                    ...
                    el.querySelectorAll(".letter").forEach(function (letter, i) {
                        setTimeout(function () {
                ...
                var bounceObserver = new IntersectionObserver(
                    function (entries) {
                        entries.forEach(function (entry) {
                            entry.target.classList.toggle("pxl-in-view", entry.isIntersecting)
                        })
                    },
                    { threshold: 0.1 }
                )
        ...
        if ($(".pxl-title--typewriter").length) {
            function typewriterOut(elements, callback) {
                if (elements.length) {
                    elements.eq(0).addClass("is-active").delay(3000).removeClass("is-active")
        ...
        $(".mc4wp-form input:checkbox").change(function () {
            $(".mc4wp-form").toggleClass("pxl-input-checked", $(this).is(":checked"))
        })
        ...
        $(".pxl-filter-sidebar .pxl-sidebar-overlay, .pxl-filter-sidebar .pxl-close-sidebar").on(
            "click",
            function () {
                $("body").removeClass("body-overflow")
    ...
    function frameflow_footer_fixed() {
        setTimeout(function () {
            ...
            var footerSel =
                typeof main_data !== "undefined" && main_data.footer_fixed_selector_footer
                    ? String(main_data.footer_fixed_selector_footer).trim()
                    : ""
            var mainSel =
                typeof main_data !== "undefined" && main_data.footer_fixed_selector_main
            ...
            if (footerSel) {
                ...
                $footer = $strip.length ? $strip : $(".pxl-footer-fixed #pxl-footer-elementor")
            }
    ...
    function frameflow_button_parallax() {
        ...
        $buttons.each(function () {
            ...
            $btn.on("mouseenter", function () {
                ...
                if ($text.length > 0) gsap.set($text, { transformOrigin: "50% 50%" })
            })
            ...
            $btn.on("mousemove", function (e) {
                ...
                requestAnimationFrame(() => {
                    ...
                    const targets = $text.length > 0 ? [$btn[0], $text[0]] : [$btn[0]]
                    gsap.to(targets, {
                        duration: 0.2,
    ...
    function frameflow_submenu_responsive() {
        ...
        $frameflow_menu.find(".pxl-menu-primary li").each(function () {
            ...
            if ($frameflow_submenu.length === 1) {
                if ($frameflow_submenu.offset().left + $frameflow_submenu.width() > winWidth) {
    ...
    function frameflow_panel_anchor_toggle() {
        ...
        $(document).on("click", ".pxl-anchor-button", function (e) {
            ...
            if ($target.hasClass("active")) {
                $target.find(".pxl-popup--conent .wow").addClass("animated").removeClass("aniOut")
                ...
                $target.find(".pxl-popup--conent .wow").addClass("aniOut").removeClass("animated")
    ...
    function frameflow_ptitle_scroll_opacity() {
        ...
        if (pxl_scroll_top <= limit) {
            $section.find(".elementor-widget").css({ opacity: 1 - pxl_scroll_top / limit })
        }
    }
    ...
    function frameflow_circle_svg(element, linearGradientId, linearGradientId1) {
        if ((window.innerWidth || document.documentElement.clientWidth) <= 1200) return
        ...
        function animateDot2(forward) {
            animation2 = Snap.animate(
                ...
                forward ? 0 : motionPathLength,
                function (val) {
                    var point = svgEl.select("path").getPointAtLength(motionPathLength - val)
    ...
    function frameflow_el_parallax() {
        $(".el-parallax-wrap").on({
            mouseenter: function () {
                $(this).addClass("hovered").find(".el-parallax-item").css({ transition: "none" })
            },
            mouseleave: function () {
            ...
            mousemove: function (e) {
                ...
                const deltaX = (bounds.left + bounds.width / 2 - e.clientX) * 0.07104
                const deltaY = (bounds.top + bounds.height / 2 - e.clientY) * 0.10656
                requestAnimationFrame(() => {
    ...
    function initTeamGridUrlState() {
        ...
        var urlDep = currentUrl ? currentUrl.searchParams.get("team_dep") : null
        var urlSearch = currentUrl ? currentUrl.searchParams.get("team_search") : null
        ...
        $grids.each(function () {
            ...
            if ($select.length) {
                $select.off(".pxlTeamUrlState").on("change.pxlTeamUrlState", function () {
                    var val = $(this).val() || ""
                    var slug = val === "" || val === "*" ? "all" : String(val).replace(/^\./, "")
                    var u = getCurrentUrl()
                    if (!u) return
                    if (slug === "all") {
            ...
            if ($search.length) {
                ...
                $search.off(".pxlTeamUrlState").on("input.pxlTeamUrlState", function () {
        ...
        $.fn.pxl_custom_type_file = function () {
            return this.each(function () {
                ...
                $file.change(function () {
                    var filename
                    if (multipleSupport) {
                        var files = Array.from($file[0].files).map(function (f) {
                            return f.name
                        })
                        filename = files.join(", ")
                    } else {
    ...
    function frameflow_zoom_point_update() {
        for (var i = 0; i < zoomPointImages.length; i++) {
            if (zoomPointImages[i].isVisible) updateZoomImage(zoomPointImages[i])
        }
    }
    ...
    function updateZoomImage(item) {
        ...
        if (elPosY > pxl_scroll_top + pxl_window_height || elPosY + elHeight < pxl_scroll_top)
            return

        var distance = pxl_scroll_top + pxl_window_height - elPosY
        var percentage = Math.min(Math.max(distance / (pxl_window_height + elHeight), 0), 1)
        item.element.style.transform = "scale(" + (1 + item.scaleAmount * percentage * 100) + ")"
    }
})(jQuery)
