...
;(function (factory) {
    ...
    function loadConfig() {
        ...
        // Firefox needs color-buffer extensions to *render* to float textures
        ;[
            "OES_texture_float",
        ...
        var extHalfLinear = gl.getExtension("OES_texture_half_float_linear")
        var configs = []

        if (isWebGL2) {
            gl.getExtension("EXT_color_buffer_float")
            gl.getExtension("EXT_color_buffer_half_float")
            // Firefox cannot LINEAR-filter 16F/32F targets — that makes
            // the height texture incomplete, so the photo shows and ripples don't.
            var webgl2Linear = !isFirefox && !!(
            ...
            if (extHalf) {
                configs.push({
                    ...
                    arrayType: null,
                    linearSupport: !!extHalfLinear && !isFirefox,
                    extensions: ["OES_texture_half_float", "EXT_color_buffer_half_float"].concat(
                        extHalfLinear ? ["OES_texture_half_float_linear"] : []
                    ),
                })
            }
            if (extFloat) {
                configs.push({
                    ...
                    internalFormat: gl.RGBA,
                    arrayType: Float32Array,
                    linearSupport: !!extFloatLinear && !isFirefox,
                    extensions: ["OES_texture_float", "WEBGL_color_buffer_float"].concat(
                        extFloatLinear ? ["OES_texture_float_linear"] : []
                    ),
                })
            }
        }

        // Always available: 8-bit render targets (Firefox WebGL1 fallback)
        configs.push({
        ...
        for (var i = 0; i < configs.length; i++) {
            var cfg = configs[i]
            var data = cfg.arrayType ? new cfg.arrayType(32 * 32 * 4) : null
            try {
                gl.texImage2D(
                    gl.TEXTURE_2D,
                    0,
    ...
    var Ripples = function (el, options) {
        ...
        this.context = gl = createGLContext(canvas)
        ...
        // Load extensions on the live context (Firefox needs this per-context)
        ...
        var textureData = arrayType
            ...
            : null
        var internalFormat = config.internalFormat || gl.RGBA
    ...
    Ripples.prototype = {
        ...
        drop: function (x, y, radius, strength) {
            ...
            if (config.type === gl.UNSIGNED_BYTE) {
                strength *= 12
            }
    ...
    $.fn.ripples = function (option) {
        if (!config) {
            throw new Error(
                "Your browser does not support WebGL or rendering to textures required for water ripples."
            )
        }
