 ...
 * Optimized + Fixed version for production
 */
...
(function ($) {
    ...
    function initLenis() {
        ...
        if (window.lenisInstance) return window.lenisInstance;
    ...
    function destroyLenis() {
        if (window.lenisInstance) {
            ...
            $(document).off('.lenisAnchor .lenisHover');

            // Destroy instance
            window.lenisInstance.destroy();
            window.lenisInstance = null;

            // Remove prevent attributes
            $('[data-lenis-prevent]').removeAttr('data-lenis-prevent');
        }
    }

    function setupAnchorLinks() {
        $(document).off("click.lenisAnchor");

        $(document).on("click.lenisAnchor", "a[href]:not(.tabs a):not(.pxl-case-badge__link)", function (e) {
            ...
            const offsetScroll = $('.pxl-header-elementor-sticky').outerHeight() || 0;

            if (lenis) {
                lenis.scrollTo($target[0], { ...CONFIG.scrollTo, offset: -offsetScroll });
            } else {
                $('html, body').stop().animate({
                    scrollTop: ($target.is('body') ? 0 : $target.offset().top - offsetScroll)
                }, 600);
            }
        });
    }
    ...
    function configureGSAP() {
        if (typeof gsap !== 'undefined' && typeof gsap.config === 'function') {
            gsap.config({ nullTargetWarn: false });
        }
    }

    function setupPreventElements() {
        CONFIG.controlSelectors.forEach(selector => {
            $(selector).each(function () {
                if (!this.hasAttribute('data-lenis-prevent')) {
    ...
    function setupHoverControls() {
        const allSelectors = CONFIG.controlSelectors.join(', ');

        $(document)
            .off("mouseenter.lenisHover mouseleave.lenisHover", allSelectors)
            .on("mouseenter.lenisHover", allSelectors, function () {
                const lenis = window.lenisInstance;
                if (lenis && typeof lenis.stop === 'function') {
                    lenis.stop();
                }
            })
            .on("mouseleave.lenisHover", allSelectors, function () {
                const lenis = window.lenisInstance;
    ...
    function handleResize() {
        ...
        resizeTimer = setTimeout(() => {
            ...
            if (isMobile && window.lenisInstance) {
                destroyLenis();
            } else if (!isMobile && !window.lenisInstance) {
                init();
            }
        }, 250);
    }
    ...
    // Public API
    window.LenisController = {
        getInstance: () => window.lenisInstance || null,

        scrollTo(target, options) {
            const lenis = window.lenisInstance;
            if (lenis && typeof lenis.scrollTo === 'function') {
                lenis.scrollTo(target, options || CONFIG.scrollTo);
            }
        },
